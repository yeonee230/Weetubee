...
export const finishGithubLogin = async (req, res) => {
  ...
  if ("access_token" in tokenRequest) {
    ...
    if (!user) {
      //디비에 깃허브 유저가 없으면 유저 계정을 create한다.
  ...
  if (!crrentPWCheck) {
    return res
      ...
      .render("users/change-password", {
        pageName,
        errorMessage: "Not match the current password.",
      });
  }
  if (newPW !== newPW2) {
    return res
      ...
      .render("users/change-password", {
        pageName,
        errorMessage: "Not match New Password and New Password Confirmation.",
      });
  }
